...
from nnunetv2.utilities.utils import get_identifiers_from_splitted_dataset_folder, \
    create_lists_from_splitted_dataset_folder, get_filenames_of_train_images_and_targets,get_filenames_of_train_images_and_targets_unsupervised
from tqdm import tqdm


class DefaultPreprocessor(object):
    def __init__(self, verbose: bool = True,supvised: bool = True):
        self.verbose = verbose
        self.supvised = supvised
        """
    ...
    def run(self, dataset_name_or_id: Union[int, str], configuration_name: str, plans_identifier: str,
        ...
        if self.supvised:
            output_directory_uns = join(nnUNet_preprocessed, dataset_name,'unsupervised', configuration_manager.data_identifier)

        if isdir(output_directory):
            shutil.rmtree(output_directory)
        if self.supvised:
            if isdir(output_directory_uns):
                shutil.rmtree(output_directory_uns)
            maybe_mkdir_p(output_directory_uns)
        maybe_mkdir_p(output_directory)
        if self.supvised:
            dataset,dataset_uns= get_filenames_of_train_images_and_targets_unsupervised(join(nnUNet_raw, dataset_name), dataset_json)
        else:
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes) as p:
            if self.supvised:
                for k in dataset_uns.keys():
                    r.append(p.starmap_async(self.run_case_save,
                                            ((join(output_directory_uns, k), dataset_uns[k]['images'],None,
            ...
            with tqdm(desc=None, total=len(r), disable=self.verbose) as pbar:
