...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
    ...
    nnunet_trainer = nnunet_trainer(plans=plans, configuration=configuration, fold=fold,
                                    dataset_json=dataset_json, unpack_dataset=not use_compressed, device=device,unsupervised=unsupervised,attention=attention,loss_method=loss_method)
...
def maybe_load_checkpoint(nnunet_trainer: nnUNetTrainer, continue_training: bool, validation_only: bool,
    ...
    if continue_training:
        ...
        if pretrained_weights_file is not None:
            ...
            load_pretrained_weights(nnunet_trainer.student_network,pretrained_weights_file,verbose=True)
        expected_checkpoint_file = None
...
def run_ddp(rank, dataset_name_or_id, configuration, fold, tr, p, use_compressed, disable_checkpointing, c, val,
            pretrained_weights, npz, val_with_best, world_size,unsupervised):
    setup_ddp(rank, world_size)
    torch.cuda.set_device(torch.device('cuda', dist.get_rank()))

    nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, tr, p,
                                           use_compressed,unsupervised=unsupervised)
    ...
    if num_gpus > 1:
        ...
        mp.spawn(run_ddp,
                 args=(
                     ...
                     val_with_best,
                     num_gpus,
                     unsupervised,attention,loss_method),
                 nprocs=num_gpus,
                 join=True)
    else:
        nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, trainer_class_name,
                                               plans_identifier, use_compressed_data, device=device,unsupervised=unsupervised,attention=attention,loss_method=loss_method)
...
def run_training_entry():
    ...
    parser.add_argument('-unsupervised',type=bool,default=False,required=False)
    parser.add_argument('-attention',type=bool,default=False,required=False)
    parser.add_argument('-loss_method',type =int,default=1,required=False)
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights,
                 ...
                 device=device,unsupervised=args.unsupervised,attention=args.attention,loss_method=args.loss_method)
