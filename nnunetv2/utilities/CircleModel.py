...
def circular_template_matching(feature_map, radius, threshold, weight_increase):
    """
    对特征图进行圆模板匹配，并增加匹配区域的权重。

    参数：
    - feature_map: 输入特征图，形状为 (batch_size, num_channels, height, width)。
    - radius: 圆模板的半径。
    - threshold: 阈值，匹配大于该值的区域。
    - weight_increase: 增加的权重值。
    ...
    circular_template = (x**2 + y**2 <= radius**2).astype(np.float32)
    circular_template = torch.from_numpy(circular_template)
    # 使用PyTorch的expand操作来复制模板并增加批次和通道维度
    expanded_template = circular_template.unsqueeze(0).unsqueeze(0)  # 添加批次和通道维度
    circular_template = expanded_template.expand(num_channels, num_channels, -1, -1)  # 扩展维度

    # 在特征图上进行滑动匹配
    matched_map = F.conv2d(feature_map, circular_template.view(num_channels, num_channels, 2*radius+1, 2*radius+1).to(feature_map.device), padding=radius)

    # 根据阈值将匹配值大于阈值的区域增加权重
    matched_map[matched_map < threshold] = 0
    feature_map = feature_map + matched_map * weight_increase
    ...
    def forward(self, skips):
        ...
        for s in range(len(self.stages)):
            ...
            if self.deep_supervision:
                    seg_outputs.append(circular_template_matching(self.seg_layers[s](x), radius=3, threshold=0.5, weight_increase=2))
            lres_input = x
