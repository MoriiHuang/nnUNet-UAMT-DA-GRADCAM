...
from nnunetv2.training.dataloading.data_loader_2d import nnUNetDataLoader2D,nnUNetDataLoader2Duns
...
class nnUNetTrainerUAMT(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda'),unsupervised:bool=False,attention:bool=False,loss_method:int =1):
        ...
        if self.unsupervised:
            print("########## You are using unsupervised")
        ...
        self.preprocessed_unsupervised_dataset_folder = join(self.preprocessed_dataset_folder_base,'unsupervised',
                                                self.configuration_manager.data_identifier)
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.student_network = self.build_network_architecture(self.plans_manager, self.dataset_json,
                                                           self.configuration_manager,
                                                           self.num_input_channels,
                                                           enable_deep_supervision=True,attention= self.attention).to(self.device)
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.SGD(self.student_network.parameters(), self.initial_lr, weight_decay=self.weight_decay,
    ...
    def get_unsupervised_datasets(self):
        case_identifiers = get_case_identifiers(self.preprocessed_unsupervised_dataset_folder)
        tr_keys = case_identifiers
        dataset_unsupervised = nnUNetDataset(self.preprocessed_unsupervised_dataset_folder, tr_keys,
                                   folder_with_segs_from_previous_stage=self.folder_with_segs_from_previous_stage,
                                   num_images_properties_loading_threshold=0)
        return dataset_unsupervised
    ...
    def get_unsupervised_dataloaders(self):
        # we use the patch size to determine whether we need 2D or 3D dataloaders. We also use it to determine whether
        # we need to use dummy 2D augmentation (in case of 3D training) and what our initial patch size should be
        patch_size = self.configuration_manager.patch_size
        dim = len(patch_size)
        ...
        dl_tr, dl_val,dl_uns= self.get_plain_dataloaders_unsupervised(initial_patch_size, dim)
        allowed_num_processes = get_allowed_n_proc_DA()
        if allowed_num_processes == 0:
            mt_gen_train = SingleThreadedAugmenter(dl_tr, tr_transforms)
            mt_gen_val = SingleThreadedAugmenter(dl_val, val_transforms)
            mt_gen_uns = SingleThreadedAugmenter(dl_uns, tr_transforms)
        else:
            print('yes')
            mt_gen_train = LimitedLenWrapper(self.num_iterations_per_epoch, data_loader=dl_tr, transform=tr_transforms,
                                             num_processes=allowed_num_processes, num_cached=6, seeds=None,
                                             pin_memory=self.device.type == 'cuda', wait_time=0.02)
            mt_gen_uns= LimitedLenWrapper(self.num_iterations_per_epoch, data_loader=dl_uns, transform=val_transforms, num_processes=max(1, allowed_num_processes // 2),
                                           num_cached=3, seeds=None, pin_memory=self.device.type == 'cuda',
                                           wait_time=0.02)
            mt_gen_val = LimitedLenWrapper(self.num_val_iterations_per_epoch, data_loader=dl_val,
                                           transform=val_transforms, num_processes=max(1, allowed_num_processes // 2),
                                           num_cached=3, seeds=None, pin_memory=self.device.type == 'cuda',
                                           wait_time=0.02)
        return mt_gen_train, mt_gen_val,mt_gen_uns
    ...
    def get_plain_dataloaders_unsupervised(self, initial_patch_size: Tuple[int, ...], dim: int):
        dataset_tr, dataset_val = self.get_tr_and_val_datasets()
        dataset_uns = self.get_unsupervised_datasets()
        ...
        if dim == 2:
            ...
            dl_val = nnUNetDataLoader2D(dataset_val, self.batch_size,
                                        ...
                                        oversample_foreground_percent=self.oversample_foreground_percent,
                                        sampling_probabilities=None, pad_sides=None)
            dl_uns = nnUNetDataLoader2Duns(dataset_uns, self.batch_size,
                self.configuration_manager.patch_size,
                self.configuration_manager.patch_size,
                self.label_manager,
                oversample_foreground_percent=self.oversample_foreground_percent,
            ...
            dl_val = nnUNetDataLoader3D(dataset_val, self.batch_size,
                                        ...
                                        oversample_foreground_percent=self.oversample_foreground_percent,
                                        sampling_probabilities=None, pad_sides=None)
            dl_uns = nnUNetDataLoader3D(
                dataset_uns, self.batch_size,
                self.configuration_manager.patch_size,
                self.configuration_manager.patch_size,
                self.label_manager,
                oversample_foreground_percent=self.oversample_foreground_percent,
    ...
    def on_train_start(self):
        ...
        self.dataloader_train, self.dataloader_val,self.dataloader_uns= self.get_unsupervised_dataloaders()
    ...
    def train_step(self, batch: dict,batch_uns:dict) -> dict:
        data = batch['data']
        target = batch['target']

        data = data.to(self.device, non_blocking=True)
        unlabeled_data=batch_uns['data'].to(self.device,non_blocking=True)
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            output = self.student_network(data)
            with torch.no_grad():
                ema_output = self.network(ema_inputs)[0]
            # del data
            l = self.loss(output, target)
        T = 8
        volume_batch_r = unlabeled_data.repeat(2, 1, 1, 1)
        stride = volume_batch_r.shape[0] // 2
        preds = torch.zeros([stride * T, 3, unlabeled_data.shape[2], unlabeled_data.shape[3]]).cuda()
        for i in range(T//2):
            ema_inputs = volume_batch_r + torch.clamp(torch.randn_like(volume_batch_r) * 0.1, -0.2, 0.2)
            with torch.no_grad():
                preds[2 * stride * i:2 * stride * (i + 1)] = self.network(ema_inputs)[0]
        preds = F.softmax(preds, dim=1)

        preds = preds.reshape(T, stride, 3, unlabeled_data.shape[2], unlabeled_data.shape[3])
        preds = torch.mean(preds, dim=0)
        uncertainty = -1.0*torch.sum(preds*torch.log(preds + 1e-6), dim=1, keepdim=True) #(batch, 1, 112,112,80)
        consistency_weight = get_current_consistency_weight(self.current_epoch//150)
        consistency_criterion = softmax_mse_loss
        consistency_dist = consistency_criterion(output[0], ema_output) #(batch, 2, 112,112,80)
        threshold = (0.75+0.25*sigmoid_rampup(self.current_epoch, 100))*np.log(2)
        mask = (uncertainty<threshold).float()
        consistency_dist = torch.sum(mask*consistency_dist)/(2*torch.sum(mask)+1e-16)
        consistency_loss = consistency_weight * consistency_dist
        l=l+consistency_loss
        if self.grad_scaler is not None:
            ...
            update_ema_variables(self.student_network, self.network, 0.99, iter_num)
        else:
            l.backward()
            torch.nn.utils.clip_grad_norm_(self.network.parameters(), 12)
            torch.nn.utils.clip_grad_norm_(self.student_network.parameters(), 12)
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            for batch_id in range(self.num_iterations_per_epoch):
                ...
                train_outputs.append(self.train_step(next(self.dataloader_train),next(self.dataloader_uns)))
